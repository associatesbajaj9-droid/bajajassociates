...
const AboutManager = () => {
  ...
  useEffect(() => {
    fetchAboutData(selectedSite);
  }, [selectedSite]);

  const fetchAboutData = async (siteId) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/about?site=${siteId}`);
      if (res.ok) {
  ...
  return (
    <div style={{ maxWidth: '800px', background: 'var(--admin-card-bg)', padding: '2rem', borderRadius: '12px', border: '1px solid var(--admin-border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        <div>
          <h2 style={{ margin: 0, color: 'var(--admin-text)', fontSize: '1.4rem' }}>Store & Footer Details</h2>
          <p style={{ margin: '0.25rem 0 0', color: 'var(--admin-text-muted)', fontSize: '0.875rem' }}>
            Manage phone, email, footer & address details for your websites.
          </p>
        </div>

        {/* Site Switcher Tabs */}
        <div style={{ display: 'flex', background: 'rgba(0,0,0,0.2)', padding: '4px', borderRadius: '8px', border: '1px solid var(--admin-border)' }}>
          <button
            type="button"
            onClick={() => setSelectedSite('site1')}
            style={{
              padding: '6px 16px',
              borderRadius: '6px',
              border: 'none',
              ...
              fontSize: '0.85rem',
              transition: 'all 0.2s',
              background: selectedSite === 'site1' ? 'var(--accent)' : 'transparent',
              color: selectedSite === 'site1' ? '#fff' : 'var(--admin-text-muted)'
            }}
          >
            Site 1 (Primary)
          </button>
          <button
            ...
            style={{
              ...
              fontSize: '0.85rem',
              transition: 'all 0.2s',
              background: selectedSite === 'site2' ? 'var(--accent)' : 'transparent',
              color: selectedSite === 'site2' ? '#fff' : 'var(--admin-text-muted)'
            }}
          >
            Site 2 (Second Project)
          </button>
        </div>
      </div>

      {/* Info notice about connecting Vercel */}
      <div style={{
      ...
      {loading ? (
        <div style={{ color: 'var(--admin-text-muted)', padding: '2rem' }}>Loading settings for {selectedSite}...</div>
      ) : (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          >
            {saving ? 'Saving Changes...' : `Save ${selectedSite === 'site1' ? 'Site 1' : 'Site 2'} Details`}
          </button>
        </form>
      )}
    </div>
